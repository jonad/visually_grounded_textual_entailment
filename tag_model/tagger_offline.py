...
class srl_img_labeler(srl_labeler):
    
    def annoatate(self, seg_file, batch, index_a, index_b, index_label,index_image, is_test=False):
        print(index_a, index_b, index_label, index_image)
        with open(seg_file, 'r', encoding='utf-8') as data_file, open(seg_file + "_tag", 'w',
                                                                      encoding='utf-8') as new_file:
            reader = csv.reader(data_file, delimiter="\t", quotechar=None)
            all_data = []
            ...
            for data_sent in tqdm(data_chunk):
                text_a = [{"sentence": sent[index_a]} for sent in data_sent]
                text_a_tags = self.post_allen(text_a)
                images = [sent[index_image] for sent in data_sent]
                text_b = [{"sentence": sent[index_b]} for sent in data_sent]
                text_b_tags = self.post_allen(text_b)
                for raw_data, tag_a, tag_b, img in zip(data_sent, text_a_tags, text_b_tags, images):
                    line = str(raw_data[0]) + "\t" + raw_data[index_a] + "\t" + raw_data[index_b] + "\t" + json.dumps(tag_a) + "\t" + json.dumps(tag_b) + "\t" + img
                    if not is_test:
                        line = line + "\t" + str(raw_data[index_label]) + "\n"
                    else:
                        line = line + "\n"
...
if __name__ == '__main__':
    ...
    # data_path = "glue_data/" + folder + "/train.tsv"
    # srl_labeler.annoatate(data_path, batch_size, index_a, index_b, index_label, has_b)
    data_path = folder + "vsnli" +"/dev.tsv"
    srl_labeler.annoatate(data_path, batch_size, index_a, index_b, index_label,index_image)
    # data_path = "glue_data/" + folder + "/test.tsv"
    # srl_labeler.annoatate(data_path, batch_size, index_a, index_b, index_label,True)
    #
    #
