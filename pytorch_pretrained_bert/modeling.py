...
if __name__=='__main__':
    ...
    #logits = model(input_ids, token_type_ids, input_mask)
    from run_classifier import TagConfig
    tag_config = TagConfig(tag_vocab_size=320000,
                           hidden_size=10,
                           layer_num=1,
                           output_dim=10,
                           dropout_prob=0.1,
                           num_aspect=3)
    #model = BertForSequenceClassificationTag(config=config, tag_config=tag_config,  num_labels=3)

    #model = BertForSequenceImgClassificationTag(config=config, tag_config=tag_config, num_labels=3,
     #                                           image_emb_size=512)
    
    #sembert_weights = set(model.state_dict().keys())
    #sembert_weights_remove = set(['pool.weight', 'pool.bias', 'classifier.weight', 'classifier.bias'])
    #nweights = sembert_weights - sembert_weights_remove
    #print(nweights)
    #print(len(nweights))
    ...
    #for k in nweights:
     #   print(k)
    print(list(model.state_dict().items()))
    # for param in model.parameters():
    #     print()
    #     print(param)
    
    #model_2 = BertForSequenceImgClassificationTag()
    #children = list(model.children())
    #print(list(model.state_dict().items())[:-4])
    #print(list(model.parameters()))
    #print(len(children))
    #print(children[6])
    # for elt in range(len(children)):
    #     print(elt)
    #     print(children[elt])
    #     print()
