...
class SequenceImageDataset(Dataset):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, start_end_idx, input_tag_ids, label_ids, input_images, transform, image_folder):
...
class InputImgFeatures(InputFeatures):
    """A single set of features of data"""
    def __init__(self, input_ids, input_mask, segment_ids, token_tag_sequence_a, token_tag_sequence_b, len_seq_a, len_seq_b, input_tag_ids, input_tag_verbs, input_tag_len, orig_to_token_split_idx, label_id, image):
        super().__init__(input_ids, input_mask, segment_ids, token_tag_sequence_a, token_tag_sequence_b, len_seq_a, len_seq_b, input_tag_ids, input_tag_verbs, input_tag_len, orig_to_token_split_idx, label_id)
        self.image = image
...
class ColaProcessor(DataProcessor):
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "train.tsv_tag_label")), "dev")
...
class QnliProcessor(DataProcessor):
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "train.tsv_tag_label")),
        "dev_matched")
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, srl_predictor):
    ...
    #label_map.update({label: i for i, label in enumerate(label_list)})
    print(label_map)
    #print(label_map)
    max_aspect = 0
    features = []
    for (ex_index, example) in enumerate(examples):
        ...
        try:
            if example.image:
                feature = InputImgFeatures(input_ids=input_ids,
...
def main():
    ...
    if task_name == "snliimg":
        model = BertForSequenceImgClassificationTag.from_pretrained(args.bert_model,cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(args.local_rank),
    ...
    if args.do_train:
        ...
        if task_name == "snliimg":
            ...
            train_data = SequenceImageDataset(all_input_ids, all_input_mask, all_segment_ids, all_start_end_idx, all_input_tag_ids,
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                #total_precision, total_recall, total_fscore = [], [], []
                ...
                with open(output_logits_file, "w") as writer:
                    ...
                    for batch_number, input_ids, input_mask, segment_ids, start_end_idx, input_tag_ids, images, label_ids in enumerate(tqdm(
                        ...
                        with torch.no_grad():
                            ...
                            logits = predict_model(input_ids, segment_ids, input_mask, start_end_idx, input_tag_ids,images,
                                                   None)
                        ...
                        precision, recall, fscore, support = precision_recall_fscore_support(label_ids, np.argmax(logits, axis=1), average='macro')
                ...
                #total_support = np.sum(np.array(total_support), axis=0)
                ...
                result = {'eval_loss': eval_loss,
                          ...
                          'loss': loss,
                          'total_precision': total_precision.tolist(),
                          'total_recall': total_recall.tolist(),
                          'total_fscore': total_fscore.tolist(),
    ...
    if args.do_eval:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for input_ids, input_mask, segment_ids, start_end_idx, input_tag_ids, images, label_ids in tqdm(
                    eval_dataloader, desc="Evaluating"):
                input_ids = input_ids.to(device)
                input_mask = input_mask.to(device)
                segment_ids = segment_ids.to(device)
                ...
                images = images.to(device)
                with torch.no_grad():
                    tmp_eval_loss = predict_model(input_ids, segment_ids, input_mask, start_end_idx, input_tag_ids,images,
                                                  label_ids)
                    logits = predict_model(input_ids, segment_ids, input_mask, start_end_idx, input_tag_ids, images,
                                           None)
                logits = logits.detach().cpu().numpy()
                ...
                eval_loss += tmp_eval_loss.mean().item()
                eval_accuracy += tmp_eval_accuracy
                precision, recall, fscore, support = precision_recall_fscore_support(label_ids, np.argmax(logits, axis=1))
                total_precision.append(precision.tolist())
                total_recall.append(recall.tolist())
                total_fscore.append(fscore.tolist())
                total_support.append(support.tolist())
            ...
            eval_loss = eval_loss / nb_eval_steps
            eval_accuracy = eval_accuracy / nb_eval_examples
            total_precision = np.sum(np.array(total_precision), axis=0) / nb_eval_examples
            total_recall = np.sum(np.array(total_recall), axis=0) / nb_eval_examples
            total_fscore = np.sum(np.array(total_fscore), axis=0) / nb_eval_examples
            if eval_accuracy > best_result:
                best_epoch = epoch
                best_result = eval_accuracy
            loss = tr_loss / nb_tr_steps if args.do_train else None
...
#     image_processor = SnliImgProcessor()
#     examples_mnli = processor.get_train_examples('glue_data/MNLI')
#     print("Examples mnli")
#     print("text a")
#     print(examples_mnli[2].text_a)
#     print("text b")
#     print(examples_mnli[2].text_b)
#     print("label")
#     print(examples_mnli[2].image)
#
#     print("Exmaple vsnli")
#     examples_vsnli = image_processor.get_dev_examples('glue_data/VSNLI')
#     print("text a")
#     print(examples_vsnli[2].text_a)
#     print("text b")
#     print(examples_vsnli[2].text_b)
#     print("label")
#     print(examples_vsnli[2].label)
#     print("image")
#     print(examples_vsnli[2].image)
#
